...
@cuda.jit
def render(pixel_loc, result, camera_origin, camera_rotation, spheres, lights, planes, triangles, amb, lamb, refl, refl_depth, aliasing):
    R = camera_rotation
    ...
    if x <= pixel_loc.shape[1] and y <= pixel_loc.shape[2]:
        ...
        # Primary Sample:
        (R, G, B) = sample(ray_origin, ray_dir, spheres, lights, planes, triangles, amb, lamb, refl, refl_depth)
        ...
        if aliasing and x+1 <= pixel_loc.shape[1] and x-1 >= 0 and y+1 <= pixel_loc.shape[2] and y-1 >= 0:
            ...
            for P in [P_left, P_right, P_top, P_bot, P_topleft, P_topright, P_bottomleft, P_bottomright]:
                ray_dir = matmul((R0, R1, R2), P)
                ray_dir = normalize(ray_dir)
                (R_s, G_s, B_s) = sample(ray_origin, ray_dir, spheres, lights, planes, triangles, amb, lamb, refl, refl_depth)
